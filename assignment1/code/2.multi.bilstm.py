...
class LSTMModel(torch.nn.Module):
    def __init__(self, embed_dim: int, lstm_dim: int, vocab_dim: int, num_labels: int):
        ...
        self.hidden_to_sarc  = torch.nn.Linear(lstm_dim * 2, num_labels)
    ...
    def forward(self, inputs: torch.tensor):
        ...
        logits_sarcasm = self.hidden_to_sarc(combined)

        return logits, logits_sarcasm
...
if __name__ == '__main__':
    ...
    parser.add_argument("--train_path_sarc", type=str, required=True, help="Path to training data.")
    parser.add_argument("--dev_path_sarc", type=str, required=True, help="Path to development data.")
    ...
    # Read the training data for sarcasm
    train_texts_sarc, train_labels_sarc = myutils.read_data(args.train_path_sarc)
    train_labels_sarc = torch.tensor(train_labels_sarc) # shape: [10000]
    train_features_sarc = convert_data(train_texts_sarc, word_vocabulary, args.max_len, update=True) # shape: [10000, 52]
    
    train_feats_batches_sarc = myutils.to_batch(train_features_sarc, args.batch_size) # shape: [156, 64, 52]
    train_labels_batches_sarc = myutils.to_batch(train_labels_sarc, args.batch_size)  # shape: [156, 64]
    ...
    # Read the dev data for sarcasm
    dev_texts_sarc, dev_labels_sarc = myutils.read_data(args.dev_path_sarc)
    dev_features_sarc = convert_data(dev_texts_sarc, word_vocabulary, args.max_len, update=False)
    dev_feats_batches_sarc = myutils.to_batch(dev_features_sarc, args.batch_size) # shape: [13, 64, 47]
    ...
    for epoch in range(args.num_epochs):
        ...
        for batch_feats, batch_labels in zip(train_feats_batches, train_labels_batches):
            optimizer.zero_grad()
            logits = lstm_model.forward(batch_feats)[0]
        ...
        # implementing a second loop for sarcasm data

        epoch_train_loss = 0.0
        for batch_feats, batch_labels in zip(train_feats_batches_sarc, train_labels_batches_sarc):
            optimizer.zero_grad()
            logits = lstm_model.forward(batch_feats)[1]
            batch_loss = loss_function(logits, batch_labels)
            batch_loss.backward()
            optimizer.step()
            epoch_train_loss += batch_loss.item()
        
        print("Train loss sarc: {:.4f}".format(epoch_train_loss/len(train_feats_batches_sarc)))
        ...
        for dev_feats in dev_feats_batches:
            pred_labels = torch.argmax(lstm_model.forward(dev_feats)[0], 1)
        ...
        # Dev sarcasm predictions
        dev_predictions = []
        for dev_feats in dev_feats_batches_sarc:
            pred_labels = torch.argmax(lstm_model.forward(dev_feats)[1], 1)
            dev_predictions.extend(pred_labels.tolist())
        score = myutils.accuracy(dev_labels_sarc, dev_predictions)
        print('Accuracy on sarc dev data: {:.2f}'.format(score))
