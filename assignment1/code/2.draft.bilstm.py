...
import myutils2
from myutils2 import Vocabulary


class LSTMModel(torch.nn.Module):
    def __init__(self, embeds, embed_dim: int, lstm_dim: int, vocab_dim: int, num_labels: int):
        """
        ...
        ----------
        embeds: tensor
            tensor containing the pre-trained embeddings
        lstm_dim: int
...
def update_vocabulary(words: List[str], word_vocabulary: Vocabulary):
    for word in words:
        word_vocabulary.getIdx(word, update=True)
    return
...
if __name__ == '__main__':
    ...
    parser.add_argument("--emb_path", type=str, required=True, help="Path to word2vec pretrained embeddings.")
    ...
    train_texts, train_labels = myutils2.read_data(args.train_path)
    train_labels = torch.tensor(train_labels) # shape: [10000]
    word_vocabulary = myutils2.Vocabulary()


    # Read the pre-trained word2vec embeddings
    words, emb_tensor, word_to_emb = myutils2.read_vecs(args.emb_path)
    update_vocabulary(words, word_vocabulary)

     
    train_features = convert_data(train_texts, word_vocabulary, args.max_len, update=True) # shape: [10000, 52]
    
    train_feats_batches = myutils2.to_batch(train_features, args.batch_size) # shape: [156, 64, 52]
    train_labels_batches = myutils2.to_batch(train_labels, args.batch_size)  # shape: [156, 64]
    
    # Read the dev data
    dev_texts, dev_labels = myutils2.read_data(args.dev_path)
    dev_features = convert_data(dev_texts, word_vocabulary, args.max_len, update=False)

    dev_feats_batches = myutils2.to_batch(dev_features, args.batch_size) # shape: [13, 64, 47]

    
    # initialize and train the model
    lstm_model = LSTMModel(emb_tensor, args.embed_dim, args.lstm_dim, len(word_vocabulary), 2)
    ...
    for epoch in range(args.num_epochs):
        ...
        score = myutils2.accuracy(dev_labels, dev_predictions)
