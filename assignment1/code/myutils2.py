...
def read_vecs(path: str):
    """
    This function reads in word embeddings in a word2vec format
    into a pytorch tensor and returns it, along with a corresponding
    list of words, which will match on index with the embeddings

    Parameters
    ----------
    path: string
        path to the word2vec source file
    ...
    words: list[string]
        list of words
    embs_tensor: tensor 
        tensor of dimensions VxE where V is the size of
        the vocabulary and E is the size of an embedding
    """
    f = open(path)
    voc_size, emb_dim = map(int, f.readline().split())
    emb_tensor = torch.empty((0, 100), dtype=torch.float32)

    # Two lists for words and embeddings
    words = []
    embs = []
    my_dict = {}

    # Get a single word and its embedding
    line = f.readline().split()
    
    # TODO
    # Is it faster to add all embeddings to a list
    # and create a tensor, or create and empty tensor
    # and add a row to it in each line with torch.cat?
    
    for i in range(500): #range(voc_size):
        line=f.readline().split()
        if not line:
            f.close()
            emb_tensor = torch.tensor(embs)
            return words, emb_tensor, my_dict
